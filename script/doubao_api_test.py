"""
Doubao Seed 2.0 API client and prompt definitions for moral dilemma experiments.
"""
...
class DoubaoAPIClient:
    """Doubao API client for Ark responses API."""

    def __init__(self, api_key: Optional[str] = None):
        self.api_key = api_key or os.getenv("ARK_API_KEY")
        if not self.api_key:
            raise ValueError("Please set DOUBAO_API_KEY in environment variables or pass api_key.")

        endpoint_id = os.getenv("DOUBAO_ENDPOINT_ID")
        self.model = endpoint_id or os.getenv("DOUBAO_MODEL", "doubao-seed-2-0-pro-260215")
            ...
            try:
                ...
                if response.status_code != 200:
                    ...
                    if response.status_code == 404:
                        error_detail = (
                            f"{error_detail} Use Ark endpoint id for model. "
                            "Set DOUBAO_ENDPOINT_ID=ep-xxxx in .env, or ensure DOUBAO_MODEL is accessible."
                        )
                ...
                if not content:
                    return {
                        ...
                        "content": None,
                        "tokens_used": 0,
                        "error": "No output text returned from Doubao responses API.",
                        "model": self.model,
                        "timestamp": datetime.now().isoformat(),
                        "finish_reason": None,
                        "elapsed_time": elapsed_time,
                ...
                finish_reason = result.get("status", "unknown")
...
def run_small_sample_experiment(num_samples: int = 1) -> Optional[List[Dict]]:
    ...
    Runs num_samples calls per cell (default 1) to verify every combination
    reaches the API without errors before launching the full matrix run.
    """
    total_cells = len(DILEMMAS) * len(RESPONSIBILITY) * len(ROLES)
    total_calls = total_cells * num_samples
    print("=" * 80)
    print("Doubao Full-Grid Smoke Test")
    ...
    output_file = os.path.join(
        output_dir,
        f"doubao_smoke_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
    )
